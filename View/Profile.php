...
<div class="col-sm-8 col-sm-offset-1">
    <?php if(isset($requestInfos)) {
        ...
        <h4><small>PAGE DE PROFIL : <?= $infos['pseudo']; ?></small></h4>
        <hr>
</div>
    ...
    <div class="infos">
        <form id="sendemail" method="post" action="modifemail.html">
            <div class="container">
                ...
                <input type="text" placeholder="<?= $infos['email']; ?>" id="email" name="email" />
        ...
        <form method="post" action="modifimage.html" enctype="multipart/form-data">
            <div class="container">
                ...
                <input type="file" placeholder="<?= $infos['image']; ?>" name="image" id="image" />
                <br>
    ...
    <div>
        ...
        <?php while ($comment = $requestComments->fetch()) {
            if($content != $comment['title']) { ?>
                <h3>
                    <?= html_entity_decode(htmlspecialchars($comment['title'])); ?>
                    <hr>
            ...
            <div class="comment container" style="margin-bottom: 20px; border: 3px solid #f1f1f1;">
                ...
                <h4><small>Posté le <?= $comment['cpost_date']; ?></small></h4>
            </div>
        <?php } ?>
    </div>
</div>
